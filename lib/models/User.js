...
module.exports = class User {
  ...
  static async updateUserPoints({ id, total_points, completed_categories }) {
    ...
    if (!user) return null;
    const updatedCategories = { ...user, total_points, completed_categories };

    const { rows } = await pool.query(
      `UPDATE users
      set total_points = $2,
      set completed_categories = ARRAY_APPEND(completed_categories, $3)
      WHERE id = $1
      RETURNING *`,
      [id,
        updatedCategories.total_points,
        updatedCategories.completed_categories]
    );
